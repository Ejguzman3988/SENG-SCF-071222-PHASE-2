...
#### 1. Persist the new image upon the `ImageForm` submission

- Send the new image data to the server using a `POST` fetch request

#### 2. Implement useEffect in App component to load images

- Import the `useEffect` hook from the React library

- Invoke `useEffect` and make a `GET` request using the `fetch` method

- Update `images` state to response from the server
