...
#### 1. Create our Dribble Clone Node Tree

#### 2. Create Components from WireFrame

- Create `ImageList`, `ImageItem`, `ImageForm`, `MainPage`, `NewImagePage`, ` ImagePreview`, components

- Have each component showcase a `div` with the name of the component 
- Connect the component with the proper parent / child relationship based on our app.

#### 3. Implement a Filter by project name feature inside the `ImageList` component: 
...
- Given the array of `images`, filter the projects that include the value of the search query

- Talk about controlling the input

#### 4. Make the `ProjectForm` component a controlled component
...
#### 5. Handle submitting the form and update state in parent using inverse data flow

- When the form is submitted:
  - Update the `images` state located in the parent component, `App` using inverse data flow
    - Use the spread operator to return a new array with the new image included
    - Set the `images` state to the new array value
